...
const ManageSubscription = () => {
    ...
    useEffect(() => {
        if (Object.keys(cancel).length > 0) {
            Swal.fire({
                title: "Konfirmasi",
                html: `<p>Apakah anda yakin membatalkan transkasi atas nama <b>${cancel?.name}</b>?</p>`,
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
        ...
        if (client?.nomor == undefined) {
            ...
            try {
                ...
                if (isSubscription) {
                    ...
                    subscriptionId = response.data.newData._id;
                    subscription.id = response.data.newData._id;
                }
    ...
    const pay = async (e) => {
        e.preventDefault();
        setLoading(true)
        var tmpSubscription = { ...detail?.subscription, paymentMethod: paymentMethod, pay: inputBayar }
        var dataForm = {
            status: "AKTIF",
    ...
    return (
        ...
        >
            <AppTable
                ...
                collection={"memberSubscription"} // Ambil data dari koleksi "cashiers"
                filter={{}} // Bisa diberikan filter
                sort={{ createdAt: -1 }} // Sortir dari terbaru
                crudAction={[]}
                listStatus={[{ name: "PAID", color: "success" }, { name: "PAYMENT", color: "warning" }, { name: "CANCEL", color: "danger" }]}
                refresh={refresh}
                setAction={setAction}
